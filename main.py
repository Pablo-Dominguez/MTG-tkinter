...
class Aplicacion():
    ...
    def createCollAux(self,event):
        ...
        try:
            ...
            df_output = pd.DataFrame(columns = ['Cardname','Times_owned','Colors','Price','Type_line'])
    ...
    def editCollection(self,event):
        ...
        if collection_text in collections_full:
            ...
            self.collection_name = [coll for coll in collections if fuzz.token_set_ratio(collection_text, coll) >0.8][0]
            text = '''
        {} {}
        _________________________________________________________
    ...
    def addCards(self,event):
        ...
        image_url = result['image_uris']['border_crop']
        response = requests.get(image_url)
        
        
        self.color = result['colors'] if result['colors'] != [] else ['X']
        self.price = float(result['prices']['eur']) if result['prices']['eur'] is not None else 0.0
        self.type_line = result['type_line']
        
        self.cardname = result['name']
        
        
        
        self.collection_df_search = self.collection_df[self.collection_df.apply(get_ratio,text = self.cardname, axis=1) > 90]
        ...
        if condition > 0:
            ...
            text = f'Cardname: {self.cardname} '+' \n ' + f'Times owned: {self.times_owned}'
            ...
            self.panel.place(x=self.width*0.25, y=self.lv_y+28+16, relwidth=0.25, relheight=0.6)
    ...
    def upNewCard(self):
        ...
        if self.row_df is None:
            a_row = [(self.cardname, 1,self.color,self.price,self.type_line)]
            #df = pd.DataFrame([[3, 4], [5, 6]])
            self.row_df = pd.DataFrame(a_row, columns=['Cardname','Times_owned','Colors','Price','Type_line'])
            ...
            self.row_df.at[self.rownumber,'Price'] = self.price
    ...
    def downCard(self):
        ...
        #self.collection_df.at[self.rownumber,'Colors'] = self.color
        self.collection_df.at[self.rownumber,'Price'] = self.price
        #self.collection_df.at[self.rownumber,'Type_line'] = self.type_line
